...
def parse_main_options(parser: ArgumentParser) -> ArgumentParser:
    ...
    # Add progress handler type argument
    parser.add_argument(
        "--progress-handler",
        choices=PROGRESS_HANDLERS.keys(),
        dest="progress_handler",
        default=DEFAULT_CONFIG["progress_handler"],
        help="The progress handler to use.",
    )
...
def parse_spotify_options(parser: ArgumentParser) -> ArgumentParser:
    ...
    # Add browsers argument
    parser.add_argument(
        "--browsers",
        nargs="*",
        default=DEFAULT_CONFIG["browsers"],
        choices=SUPPORTED_BROWSERS,
        help="The browsers used when getting cookies",
    )
...
def parse_output_options(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=len(sys.argv) > 1 and sys.argv[1] == "save",
    )
